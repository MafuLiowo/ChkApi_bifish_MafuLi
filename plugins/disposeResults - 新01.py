...
try:
    # 指定编码为 utf-8 打开文件
    with open(os.path.join(os.path.dirname(__file__), './plugins/rule.yaml'), 'r', encoding='utf-8') as file:
        hae_rule = yaml.safe_load(file)
except Exception as e:
    # 指定编码为 utf-8 打开文件
    with open('./plugins/rule.yaml', 'r', encoding='utf-8') as file:
        hae_rule = yaml.safe_load(file)

try:
    # 指定编码为 utf-8 打开文件，并使用 os.path.join 确保路径正确
    with open(os.path.join(os.path.dirname(__file__), './plugins/sensitive_data_rule.yaml'), 'r', encoding='utf-8') as file:
        sensitive_data_rule = yaml.safe_load(file)
except Exception as e:
    # 指定编码为 utf-8 打开文件
    with open('./plugins/sensitive_data_rule.yaml', 'r', encoding='utf-8') as file:
        sensitive_data_rule = yaml.safe_load(file)
